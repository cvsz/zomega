...
async def require_admin(x_zomega_admin_token: str = Header(..., alias="X-zomega-Admin-Token")) -> dict:
    if not secrets.compare_digest(x_zomega_admin_token, settings.zomega_admin_token):
        raise HTTPException(401, "Invalid admin token")
    return {"actor_type": "admin", "actor_id": "zomega-admin"}
