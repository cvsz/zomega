...
def list_audit_events(tenant_id: str, limit: int = 100) -> list[dict]:
    ...
    return _query_audit_events(tenant_id, bounded)

def _query_audit_events(tenant_id: str, limit: int) -> list[dict]:
    with session_scope() as db:
        rows = db.execute(
            select(AuditEvent)
...
def export_audit_ndjson(tenant_id: str, limit: int = 10000):
    bounded = min(max(limit, 1), 10000)
    events = _query_audit_events(tenant_id, bounded)
    for event in events:
