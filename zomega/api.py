...
app = FastAPI(title="zomega Production API", version="3.0.0")
RUNS = Counter("zomega_runs_created_total", "zomega runs created", ["kind"])
...
def audit_export(limit: int = 10000, tenant=Depends(require_audit_read)):
    return StreamingResponse(
        ...
        headers={"Content-Disposition": "attachment; filename=zomega-audit.ndjson"},
    )
