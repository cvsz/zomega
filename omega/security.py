...
def generate_api_key() -> str:
    ...
    return f"omega_{locator}_{secret}"

def parse_api_key(raw: str) -> tuple[str, str]:
    if not raw.startswith("omega_"):
        raise ValueError("Invalid OMEGA API key format")
    parts = raw.split("_", 2)
    if len(parts) != 3:
        raise ValueError("Invalid OMEGA API key format")
    _, locator, secret = parts
    if len(locator) != 24 or any(ch not in "0123456789abcdef" for ch in locator):
        raise ValueError("Invalid OMEGA API key locator")
    if len(secret) < 32:
        raise ValueError("OMEGA API key secret is too short")
    return locator, secret

def _argon2_material(secret: str) -> str:
    return f"{secret}:{settings.omega_api_key_pepper}"
